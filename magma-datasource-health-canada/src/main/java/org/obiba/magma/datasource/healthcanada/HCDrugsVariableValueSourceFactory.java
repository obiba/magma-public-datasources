...
public class HCDrugsVariableValueSourceFactory implements VariableValueSourceFactory {
  ...
  private ImmutableSet.Builder<VariableValueSource> valueSourceBuilder;

  @Override
  public Set<VariableValueSource> createSources() {
    valueSourceBuilder = ImmutableSet.builder();

    addDrugVariables();
    addIngredVariables();
    addFormVariables();
    addStatusVariables();
    addPackagingVariables();
    addPharmVariables();
    addRouteVariables();
    addScheduleVariables();
    addTherVariables();
    addCompVariables();

    return valueSourceBuilder.build();
  }

  private void addDrugVariables() {
    // drug
    valueSourceBuilder.add(newVariableValueSource(
    ...
    valueSourceBuilder.add(newVariableValueSource(
        ...
        "drug.txt", 3));
    valueSourceBuilder.add(
        newVariableValueSource(newVariable("BRAND_NAME").addAttribute("label", "Brand Name", Locale.ENGLISH),
            "drug.txt", 4));
    valueSourceBuilder.add(
        newVariableValueSource(newVariable("DESCRIPTOR").addAttribute("label", "Description", Locale.ENGLISH),
            "drug.txt", 5));
    valueSourceBuilder.add(newVariableValueSource(
  ...
  private void addIngredVariables() {
    // ingred
    valueSourceBuilder.add(newVariableValueSource(newVariable("ACTIVE_INGREDIENT_CODE", TextType.get(), "ingred")
    ...
    valueSourceBuilder.add(newVariableValueSource(
        ...
        "ingred.txt", 2));
    valueSourceBuilder.add(newVariableValueSource(newVariable("INGREDIENT_SUPPLIED_IND", TextType.get(), "ingred")
  ...
  private void addFormVariables() {
    // form
    valueSourceBuilder.add(newVariableValueSource(newVariable("PHARM_FORM_CODE", TextType.get(), "form")
        .addAttribute("label", "Pharmaceutical Form Code", Locale.ENGLISH), "form.txt", 1));
    valueSourceBuilder.add(newVariableValueSource(newVariable("PHARMACEUTICAL_FORM", TextType.get(), "form")
        .addAttribute("label", "Pharmaceutical Form", Locale.ENGLISH), "form.txt", 2));
  }

  private void addStatusVariables() {
    // status
    valueSourceBuilder.add(newVariableValueSource(newVariable("CURRENT_STATUS_FLAG", TextType.get(), "status")
  ...
  private void addPackagingVariables() {
    // package
    valueSourceBuilder.add(newVariableValueSource(
        newVariable("UPC", TextType.get(), "package").addAttribute("label", "Universal Product Code", Locale.ENGLISH),
        "package.txt", 1));
    valueSourceBuilder.add(newVariableValueSource(newVariable("PACKAGE_SIZE_UNIT", TextType.get(), "package")
    ...
    valueSourceBuilder.add(newVariableValueSource(
        ...
        "package.txt", 4));
    valueSourceBuilder.add(newVariableValueSource(newVariable("PRODUCT_INFORMATION", TextType.get(), "package")
        .addAttribute("label", "Product Information", Locale.ENGLISH), "package.txt", 5));
  }

  private void addPharmVariables() {
    // pharm
    valueSourceBuilder.add(newVariableValueSource(newVariable("PHARMACEUTICAL_STD", TextType.get(), "pharm")
        .addAttribute("label", "Pharmaceutical STD", Locale.ENGLISH), "pharm.txt", 1));
  }

  private void addRouteVariables() {
    // route
    valueSourceBuilder.add(newVariableValueSource(newVariable("ROUTE_OF_ADMINISTRATION_CODE", TextType.get(), "route")
        .addAttribute("label", "Route of Administration Code", Locale.ENGLISH), "route.txt", 1));
    valueSourceBuilder.add(newVariableValueSource(newVariable("ROUTE_OF_ADMINISTRATION", TextType.get(), "route")
        .addAttribute("label", "Route of Administration", Locale.ENGLISH), "route.txt", 2));
  }

  private void addScheduleVariables() {
    // schedule
    valueSourceBuilder.add(newVariableValueSource(
  ...
  private void addTherVariables() {
    ...
    valueSourceBuilder.add(newVariableValueSource(
        ...
        2));
    valueSourceBuilder.add(newVariableValueSource(newVariable("TC_AHFS_NUMBER", TextType.get(), "ther")
        .addAttribute("label", "American Hospital Formulary Service Number", Locale.ENGLISH), "ther.txt", 3));
    valueSourceBuilder.add(newVariableValueSource(newVariable("TC_AHFS", TextType.get(), "ther")
        .addAttribute("label", "American Hospital Formulary Service", Locale.ENGLISH), "ther.txt", 4));
  }

  private void addCompVariables() {
    // comp
    valueSourceBuilder.add(
    ...
    valueSourceBuilder.add(
        newVariableValueSource(newVariable("COUNTRY").addAttribute("label", "Country", Locale.ENGLISH), "comp.txt",
            13));
    valueSourceBuilder.add(
        newVariableValueSource(newVariable("POSTAL_CODE").addAttribute("label", "Postal Code", Locale.ENGLISH),
            "comp.txt", 14));
    valueSourceBuilder.add(
  ...
  private VariableValueSource newVariableValueSource(Variable.Builder variableBuilder, String sourceFile, int column) {
    return new HCDrugsVariableValueSource(
        variableBuilder.addAttribute("file", sourceFile).addAttribute("column", Integer.toString(column)));
  }
  ...
  private Variable.Builder newVariable(String name, ValueType type, String occGroup) {
    Variable.Builder variableBuilder = Variable.Builder.newVariable(name, type, HCDrugsValueTable.DRUG_ENTITY_TYPE);
    if(occGroup != null) {
      variableBuilder.repeatable();
      if(!occGroup.isEmpty()) {
        variableBuilder.occurrenceGroup(occGroup);
      }
    }
    variableBuilder.addAttribute("info", "http://www.hc-sc.gc.ca/dhp-mps/prodpharma/databasdon/index-eng.php");
    variableBuilder.addAttribute("source", HCDrugsValueTable.ALL_FILES_ZIP_URL);

    return variableBuilder;
  }

  private Category newCategory(String name, String label) {
    return Category.Builder.newCategory(name).addAttribute("label", label, Locale.ENGLISH).build();
  }
